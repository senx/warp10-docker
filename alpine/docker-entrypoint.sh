...
if [ -e "${FIRSTINIT_FILE}" ]; then
  ...
  if [ ! -e "${WARP10_HOME}"/etc/initial.tokens ]; then
    su-exec warp10 java -cp "${WARP10_HOME}"/bin/warp10-"${WARP10_VERSION}".jar -Dfile.encoding=UTF-8 io.warp10.worf.TokenGen "${WARP10_HOME}"/etc/conf.d/00-secrets.conf "${WARP10_HOME}"/etc/conf.d/00-warp.conf "${WARP10_HOME}"/templates/warp10-tokengen.mc2 "${WARP10_HOME}"/etc/initial.tokens
