...
then
  ...
  if ! grep -q  REVISION_TAG ${WARP10_VOLUME}/warp10/etc/conf-standalone.conf; then
     ...
     sed -i 's_/opt/warp10-1\.0\.[0-6]\(-[0-9]+-[a-z0-9]+\)*_\$\{standalone\.home\}_g' ${WARP10_VOLUME}/warp10/etc/conf-standalone.conf
     ...
     sed -i 's_/opt/warp10-1\.0\.[0-6]\(-[0-9]+-[a-z0-9]+\)*_/opt/warp10_g'  ${WARP10_VOLUME}/warp10/etc/log4j.properties
  ...
  echo "Configuration File exists - launch Warp10"
